    ...
    <table class="table table-bordered">
        ...
        @forelse($origins as $key => $origin)
            <tr>
                ...
                <td>{{ $origin->slug }}</td>
                {{-- <td>
                    <img src="{{ asset($origin->image) }}"height="30"width="120"alt="{{ ($origin->image) }}">
                </td> --}}
                <td>
                    <div class="btn-group btn-group-mini btn-corner">

                            <a href="{{ route('backend.product.origins.edit', $origin->id) }}"class="btn btn-xs btn-info"
                            title="Edit"><i class="ace-icon fa fa-pencil"></i>
                            </a>

                            <button type="button" class="btn btn-xs btn-danger"onclick="delete_check({{$origin->id}})"
